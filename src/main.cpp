...
uint256 hashGenesisBlock("0x5164f51f8d38c57fb0663ce94537b60029a38a623cf0f9bb6808091a68198a94");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Domicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Domicoin Signed Message:\n";
...
{
    // Domicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Domicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Domicoin: 3.5 days
static const int64 nTargetSpacing = 60; // Domicoin: 1 minute	
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Domicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Domicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Domicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5164f51f8d38c57fb0663ce94537b60029a38a623cf0f9bb6808091a68198a94");
    }
...
bool InitBlockIndex() {
	...
	// Genesis block:
 	...
 	// vMerkleTree: 5a2e19825b
		...
		const char* pszTimestamp = "replace with something that ensures no premining took place";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x3a24d3e1fb88ab86e7a416211f86c9b162badaddac104c9dad18eae0a9f24b77"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Domicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DomicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DomicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DomicoinMiner(CWallet *pwallet)
{
    printf("DomicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("domicoin-miner");
    ...
    try { loop {
        ...
        printf("Running DomicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DomicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DomicoinMiner, pwallet));
}
